...
function buildFallbackCarousel(lang: 'en' | 'ar'): HeroCarouselItem[] {
  return [
    {
      title: lang === 'en' ? 'New Partnership in Jordan' : 'Ø´Ø±Ø§ÙƒØ© Ø¬Ø¯ÙŠØ¯Ø© ÙÙŠ Ø§Ù„Ø£Ø±Ø¯Ù†',
      category: lang === 'en' ? 'News' : 'Ø£Ø®Ø¨Ø§Ø±',
      date: '2024',
      image: 'https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?auto=format&fit=crop&q=80&w=800',
    },
    {
      title: lang === 'en' ? 'Strategic Infrastructure Project' : 'Ù…Ø´Ø±ÙˆØ¹ Ø¨Ù†ÙŠØ© ØªØ­ØªÙŠØ© Ø§Ø³ØªØ±Ø§ØªÙŠØ¬ÙŠ',
      category: lang === 'en' ? 'Project' : 'Ù…Ø´Ø±ÙˆØ¹',
      date: '2024',
      image: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=800',
    },
    {
      title: lang === 'en' ? 'Consulting Excellence Award' : 'Ø¬Ø§Ø¦Ø²Ø© Ø§Ù„ØªÙ…ÙŠØ² ÙÙŠ Ø§Ù„Ø§Ø³ØªØ´Ø§Ø±Ø§Øª',
      category: lang === 'en' ? 'Award' : 'Ø¬Ø§Ø¦Ø²Ø©',
      date: '2023',
...
export const Hero: React.FC<HeroProps> = ({ lang }) => {
  ...
  useEffect(() => {
    ...
    fetchArticles().then((posts) => {
      ...
      const fromApi: HeroCarouselItem[] = sorted.slice(0, 6).map((p) => ({
        title: p.title,
        category: p.categories[0] ?? (lang === 'en' ? 'Blog' : 'Ø§Ù„Ù…Ø¯ÙˆÙ†Ø©'),
  ...
  const socials = [
    { icon: <Linkedin className="h-4 w-4" />, href: '#', color: 'hover:text-[#0077b5]', label: lang === 'en' ? 'LinkedIn' : 'Ù„ÙŠÙ†ÙƒØ¯Ø¥Ù†' },
    { icon: <Twitter className="h-4 w-4" />, href: '#', color: 'hover:text-[#1DA1F2]', label: lang === 'en' ? 'Twitter' : 'ØªÙˆÙŠØªØ±' },
    { icon: <Facebook className="h-4 w-4" />, href: '#', color: 'hover:text-[#4267B2]', label: lang === 'en' ? 'Facebook' : 'ÙÙŠØ³Ø¨ÙˆÙƒ' },
    { icon: <Instagram className="h-4 w-4" />, href: '#', color: 'hover:text-[#E1306C]', label: lang === 'en' ? 'Instagram' : 'Ø¥Ù†Ø³ØªØºØ±Ø§Ù…' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-8 sm:space-y-10">
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_minmax(300px,40%)] lg:min-h-[min(88vh,640px)]">
            {/* Ø¹Ù…ÙˆØ¯ Ø§Ù„Ù†Øµ */}
            <div className="flex flex-col justify-center border-black/[0.06] p-8 sm:p-10 lg:border-e lg:p-12 xl:p-14">
              ...
              >
                <span className="inline-flex items-center gap-2 rounded-full border border-primary/15 bg-primary/[0.06] px-3 py-1.5 text-label font-bold uppercase tracking-[0.22em] text-primary">
                  ...
                  {lang === 'en' ? 'Consulting Excellence' : 'ØªÙ…ÙŠØ² Ø§Ø³ØªØ´Ø§Ø±ÙŠ'}
                </span>
              </motion.div>
                  ...
                  >
                    {item.href ? (
                      <Link
                        ...
                        aria-label={
                          lang === 'en' ? `Open article: ${item.title}` : `ÙØªØ­ Ø§Ù„Ù…Ù‚Ø§Ù„: ${item.title}`
                        }
                      >
                        <img
                          src={item.image}
                    ...
                    <div className="absolute inset-x-0 bottom-0 z-20 px-6 pb-8 pt-1 sm:px-8 sm:pb-10">
                      <div
                        ...
                        role="tablist"
                        aria-label={lang === 'en' ? 'Highlight slides' : 'Ø´Ø±Ø§Ø¦Ø­ Ø§Ù„Ù…Ù…ÙŠØ²Ø§Øª'}
                      >
        ...
        {/* Ø¥Ø­ØµØ§Ø¦ÙŠØ§Øª â€” Ø¨Ø¯ÙˆÙ† Ø¨Ø·Ø§Ù‚Ø§ØªØ› ÙÙˆØ§ØµÙ„ Ø±Ù…Ø§Ø¯ÙŠØ© */}
        <motion.div className="flex flex-col divide-y divide-dark/12 md:flex-row md:divide-y-0 md:divide-dark/12 md:divide-x rtl:md:divide-x-reverse">
          <motion.div
          ...
          >
            ...
            <p className="text-meta font-medium italic leading-relaxed text-dark/45">
              {lang === 'en'
                ...
                : 'ØªÙ‚Ø¯ÙŠÙ… Ø®Ø¯Ù…Ø§Øª Ø§Ø³ØªØ´Ø§Ø±ÙŠØ© Ø¹Ø§Ù„ÙŠØ© Ø§Ù„Ø¬ÙˆØ¯Ø© Ø¨Ø§Ø³ØªÙ…Ø±Ø§Ø±.'}
            </p>
          </motion.div>
          ...
          >
            <div className="flex w-full flex-col gap-5 sm:flex-row sm:items-center sm:justify-between" dir="ltr">
              <div className={cn('min-w-0 flex-1', lang === 'ar' ? 'text-right' : 'text-left')}>
                <p className="mb-1 text-meta font-bold text-dark/35">{t.stats[2].label}</p>
                <p className="text-lg font-black leading-tight tracking-tighter text-dark sm:text-xl">{t.stats[2].value}</p>
              </div>
              <div className="flex shrink-0 justify-center gap-3 sm:gap-4">
                <div className="flex h-[4.25rem] w-[4.25rem] items-center justify-center sm:h-20 sm:w-20">
                  <img
                    src={palestineImg}
                    alt={lang === 'en' ? 'Palestine' : 'ÙÙ„Ø³Ø·ÙŠÙ†'}
                ...
                <div className="flex h-[4.25rem] w-[4.25rem] items-center justify-center sm:h-20 sm:w-20">
                  <img
                    src={jordanImg}
                    alt={lang === 'en' ? 'Jordan' : 'Ø§Ù„Ø£Ø±Ø¯Ù†'}
                    className="max-h-full max-w-full object-contain p-1.5"
                  />
                </div>
              </div>
